...
<h2 class="<?php echo esc_attr( $heading_class ); ?>" style="text-transform: uppercase;">
	<?php
	if ( $email_improvements_enabled ) {
...
<div style="margin-bottom: <?php echo $email_improvements_enabled ? '24px' : '40px'; ?>; background-color: #F4F4F4; padding: 25px;">
	<table class="td font-family <?php echo esc_attr( $order_table_class ); ?>" cellspacing="0" cellpadding="6" style="width: 100%;" border="1">
		...
		<tfoot>
			...
			<tr class="order-totals">
				<th class="td text-align-left" scope="row" colspan="2"><?php esc_html_e( 'Payment Status:', 'woocommerce' ); ?></th>
				<td class="td text-align-right"><?php echo esc_html( wc_get_order_status_name( $order->get_status() ) ); ?></td>
			</tr>
		</tfoot>
	</table>
</div>
